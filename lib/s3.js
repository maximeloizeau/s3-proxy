...
module.exports = function(options) {
  ...
  function getObject(req, res, next) {
    var finalPath = req.originalUrl;
    if(
      options.directoryIndex &&
      req.originalUrl.slice(-1) === '/'
    ) {
      finalPath = req.originalUrl + options.directoryIndex;
    }

    // This will get everything in the path following the mountpath
    var s3Key = decodeURIComponent(finalPath.substr(req.baseUrl.length + 1));
  ...
  return function(req, res, next) {
    ...
    var enableListing = !options.directoryIndex;
    if (enableListing && req.path.slice(-1) === '/') {
